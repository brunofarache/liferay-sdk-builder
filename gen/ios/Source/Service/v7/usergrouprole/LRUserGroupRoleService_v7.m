...
- (void)updateUserGroupRolesWithUserId:(long long)userId groupId:(long long)groupId addedRoleIds:(NSArray *)addedRoleIds deletedRoleIds:(NSArray *)deletedRoleIds error:(NSError **)error {
	NSMutableDictionary *_params = [NSMutableDictionary dictionaryWithDictionary:@{
		@"userId": @(userId),
		@"groupId": @(groupId),
		@"addedRoleIds": [self checkNull: addedRoleIds],
		@"deletedRoleIds": [self checkNull: deletedRoleIds]
	}];

	NSDictionary *_command = @{@"/usergrouprole/update-user-group-roles": _params};
